...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    setLoading(true)
    emailjs.send("service_tarzno8" ,"template_ey368ki" , {
      from_name:form.name,
      to_name:"Harsh",
      from_email:form.email,
      to_email:"harshsik7890@gmail.com",
      message:form.message,
    } , "2R3217AF6eFT4Q3bX").then(()=>{
      setLoading(false)
      ...
      <motion.div variants={slideIn("left" , "tween" , 0.2 , 1)} className="flex-[0.75] bg-black-100 p-8 rounded-2xl">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="mt-12 flex flex-col gap-8">
          ...
          <label className="flex flex-col">
            ...
            <textarea rows={7} type="message" name="message" value={form.message} onChange={handleChange} placeholder="What do you want to say?" className="bg-tertiary py-4 px-6 placeholder:text-secondary text-white rounded-lg outline-none border-none font-medium"/>
          </label>

          <button 
          type="submit" className="bg-tertiary py-3 px-8 outline-none w-fit text-white font-bold shadow-md shadow-primary rounded-lg" 
          >
