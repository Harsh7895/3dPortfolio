    ...
    >
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        <Link to={"/"} className='flex items-center gap-2' onClick={() => {
          ...
          <img src={logo} alt="logo" className='h-12 w-12 object-contain' id='logo' />
          <p className='text-white text-[18px] font-bold cursor-pointer' >Harsh <span className='hidden sm:block'>| Developer</span></p>
        </Link>
        ...
        <div className={`${!toggle ? 'hidden' : 'flex'} sm:hidden p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
          <ul className='list-none flex justify-end items-start flex-col gap-4'>
            {navLinks.map((Link) => (
              <li key={Link.id} className={`${active === Link.title ? "text-white" : "text-secondary"}
          font-poppins font-medium cursor-pointer text-[16px] hover:text-white`} onClick={() => settoggle(false)}>
